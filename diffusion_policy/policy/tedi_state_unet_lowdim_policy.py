...
from diffusion_policy.model.diffusion.conditional_unet1d_tedi_state import StatefulConditionalUnet1D
...
class TEDiStateUnetLowdimPolicy(BaseLowdimPolicy):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        # Past action horizon and buffer
        self.past_action_horizon = kwargs.get("past_action_horizon", 4)
        self.past_action_weight = kwargs.get("past_action_weight", 0.5)
    ...
    def reset_buffer(self):
        self.action_buffer = None
        self.buffer_diff_steps = None
        self.past_action_buffer = None              # New
    ...
    #  New get past action func
    def get_past_action_cond(self):
        if self.past_action_buffer is None:
            return None
        # Take last N steps of shape (B, T, Da)
        k = self.past_action_horizon
        return self.past_action_buffer[:, -k:]  # shape: (B, k, Da)
        ...
        **kwargs,
        ...
        if self.action_buffer is None:
            ...
            for t in scheduler.timesteps.to(device=self.device, dtype=torch.long):
                ...
                past_action_cond = self.get_past_action_cond()      # New!
                model_output = self.model(
                    trajectory,
                    t.repeat(B, Tp),
                    local_cond=local_cond,
                    global_cond=global_cond,
                    past_action_cond=past_action_cond,
        ...
        **kwargs,
        ...
        while torch.max(self.buffer_diff_steps[:, (To - 1) + Ta - 1]) != -1:
            ...
            model_output = model(
                ...
                local_cond=local_cond,
                global_cond=global_cond,
                past_action_cond=past_action_cond,  # NEW ARG
            ...
            self.action_buffer = scheduler.step(
                ...
                timestep=self.buffer_diff_steps,
    ...
    def predict_action(
        ...
        # New! Extract past actions if available
        ...
        # New! Produce clean action chunk
        past_action_cond = self.get_past_action_cond()
        ...
        nsample = self.conditional_sample(
            ...
            local_cond=local_cond,
            global_cond=global_cond,
            past_action_cond=past_action_cond,  # New
            **self.kwargs,
        )
        ...
        # New! Store past actions for conditioning
        if self.past_action_buffer is None:
            self.past_action_buffer = action.clone()
        else:
            self.past_action_buffer = torch.cat(
                [self.past_action_buffer, action], dim=1
            )[:, -self.past_action_horizon:]  # keep latest k
    ...
    def compute_loss(self, batch):
        ...
        if hasattr(self, "past_action_horizon"):
            past_action_cond = action[:, :self.past_action_horizon]  # use early part as past
        else:
            past_action_cond = None

        # Predict the denoised image from t_1
        pred = self.model(
            noisy_trajectory, diff_steps, local_cond=local_cond, global_cond=global_cond, past_action_cond=past_action_cond,
        )
...
if __name__ == "__main__":
    ...
    policy = TEDiStateUnetLowdimPolicy(
        model=model,
